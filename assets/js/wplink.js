...
( function( $, wpLinkL10n, wp ) {
    ...
    wpLink = {
        ...
        init: function() {
            ...
            // Advanced WPLink
            var top_distance = 50;
            if(awl_title=='enabled'){
                var n_nof = '<div class="link-title"><label><span>'+wpLinkL10n.linktitle+'</span> <input type="text" placeholder="'+wpLinkL10n.linktitle_desc+'" id="link-title" /></label></div>';
                jQuery(n_nof).insertAfter("#link-options .wp-link-text-field");
                top_distance = top_distance+27;
            }
            if(awl_linkstyles.length>=1){
                var n_styles = '<div class="wp-link-styling-field"><label><span><b>Styling</b></span> <select id="customstyles" style="width:70%" ><option value="">None</option></select></label></div>';
                jQuery(n_styles).insertAfter("#link-options .link-target");
                jQuery.each(awl_linkstyles, function(key,vals) {
                    jQuery('#wp-link select#customstyles').append('<option value="'+vals.selector+'">'+vals.name+'</option>');
                });
                top_distance = top_distance+27;
            }
            if(awl_rel=='enabled'){
                var n_nof = '<div class="link-follow"><label><span></span> <input type="checkbox" id="rel-nofollow-checkbox" /> rel="nofollow"</label></div>';
                jQuery(n_nof).insertAfter("#link-options .link-target");
                top_distance = top_distance+23;
            }
            // End Advanced WPLink

            var top = jQuery(".query-results").css("top");
            jQuery(".query-results").css("top", parseInt(top)+top_distance);
            ...
            inputs.fsCustomStyles = $( 'select#customstyles' );
            inputs.fsLinkTitle = $('input#link-title');
            // End Advanced WPLink
